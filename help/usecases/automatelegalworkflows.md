...
此方案使用“条款和条件”文档，该文档可以下载 [此处](https://github.com/benvanderberg/adobe-document-generation-samples/blob/main/Agreement/exercise/TermsAndConditions_Sample.docx?raw=true).

![条款和条件文档的屏幕截图](assets/automatelegal_3.png)

1. 打开 *TermsAndConditions.docx* Microsoft Word中的示例文档。
1. 如果 [Document Generation](https://appsource.microsoft.com/en-cy/product/office/WA200002654) 插件已安装，请选择 **[!UICONTROL Document Generation]** 在“功能区”中。 如果在功能区中看不到Document Generation，请按照以下说明进行操作。
1. 选择 **[!UICONTROL 开始使用]**.
1. 将上面写入的JSON示例数据复制到“JSON数据”字段中。

   ![文档和JSON数据的屏幕快照](assets/automatelegal_4.png)
...
1. 向内 *Document Generation Tagger*，搜索“name”。
1. 在“公司”下，选择 *名称*.
...
![在页脚中添加COMPANY和CUSTOMER标签的屏幕截图](assets/automatelegal_6.png)

最后，您需要 **重复步骤1-4** 要将签名页面中“客户”部分下的“名字”和“姓氏”替换为以下各项的标签： `{{customer.signer.firstName}}` 和 `{{customer.signer.lastName}}` ，而其他组别则分别为。 如果标签很长，并且由于在生成文档时替换了标签而重排到下一行，请不要担心。
...
![页脚的屏幕快照](assets/automatelegal_8.png)
...
1. 向内 *Document Generation Tagger*，选择 **[!UICONTROL 生成文档]**.
1. 系统首次可能会提示您使用Adobe ID登录。 选择 **[!UICONTROL 登录]** 并完成使用您的凭据登录的提示。
...
![选择“加州信息披露条款”部分的屏幕截图](assets/automatelegal_14.png)

1. 在文档中，选择“加州信息披露条款”部分和所有子项目符号。

   ![Conditional-section标记的屏幕截图](assets/automatelegal_15.png)

1. 向内 *[!UICONTROL Document Generation Tagger]*，选择 **[!UICONTROL 高级]**.
1. 展开 **[!UICONTROL 条件内容]**.
1. 向内 *[!UICONTROL 选择记录]* 字段，搜索和选择 **[!UICONTROL customer.state]**.
1. 向内 *[!UICONTROL 选择运算符]* 字段，选择 **=**.
1. 向内 *[!UICONTROL 值]* 字段，类型 *CA*.
1. 选择 **[!UICONTROL 插入条件]**.
...
![加利福尼亚州信息披露信息的屏幕截图](assets/automatelegal_18.png)
...
1. 向内 *Document Generation Tagger*，选择 **[!UICONTROL 编辑输入数据]**.

   ![Document Generation Tagger的屏幕截图](assets/automatelegal_20.png)

1. 选择 **[!UICONTROL 编辑]**.

1. 在JSON数据中，更改 *CA* 收件人 *WA*.
...
![仅包含Wasington状态部分的文档的屏幕快照](assets/automatelegal_22.png)
...
1. 向内 *[!UICONTROL Document Generation Tagger]*，选择 **[!UICONTROL 高级]**.
1. 展开 **[!UICONTROL 条件内容]**.
1. 下 *[!UICONTROL 内容类型]*，选择 **[!UICONTROL 短语]**.
1. 向内 *[!UICONTROL 选择记录]* 字段，搜索和选择 **[!UICONTROL customer.state]**.
1. 向内 *[!UICONTROL 选择运算符]* 字段，选择 **=**.
1. 向内 *[!UICONTROL 值]* 字段，类型 *CA*.
1. 选择 **[!UICONTROL 插入条件]**.
...
1. 将光标放在需要放置签名的位置。
   ...
   ![签名需要转到哪个位置的屏幕截图](assets/automatelegal_24.png)

1. 向内 *[!UICONTROL Document Generation Tagger]*，选择 **[!UICONTROL Adobe Sign]**.
1. 向内 *[!UICONTROL 指定收件人数]* 字段，设置收件人的数量（本示例使用2）。
1. 向内 *[!UICONTROL 收件人]* 字段，选择 **[!UICONTROL Signer-1]**.
1. 向内 *[!UICONTROL 字段]* 类型，选择 **[!UICONTROL 签名]**.
...
放置的Date标记相当长： `{{Date 3_es_:signer1:date:format(mm/dd/yyyy):font(size=Auto)}}`. Acrobat Sign文本标记必须保留在同一行，这与Document Generation标记不同。 该 `:format()` 和 `font()` 参数是可选的，因此对于这种情况，我们可以将标记缩短为 `{{Date 3_es_:signer1:date}}`.
...
1. 打开您的 **[!UICONTROL 终端]** 安装依赖关系的方法 `npm install`.
1. 复制样本 *data.json* 到 *resources* 文件夹。
1. 将您创建的Word模板复制到 *resources* 文件夹。
1. 在samples文件夹的根目录下创建一个名为 *generate-salesOrder.js*.
...
1. 替换 `<JSON FILE>` /resources中的JSON文件的名称。
1. 替换 `<INSERT DOCX>` DOCX文件的名称。
1. 要运行，请使用 **[!UICONTROL 终端]** 执行节点 `generate-salesOrder.js`.
...
如果您没有Acrobat Sign帐户，请注册一个开发人员帐户并查看文档 [此处](https://developer.adobe.com/adobesign-api/)，然后选择 **开发人员帐户注册**. 系统会提示您填写表单并接收验证电子邮件。 完成此操作后，您将定向到一个网站以设置您的密码和帐户，然后您可以在其中登录Acrobat Sign。

## 从Web界面发送协议

1. 选择 **[!UICONTROL 发送]** 从导航栏中打开。

   ![Acrobat Sign中“发送”选项卡的屏幕截图](assets/automatelegal_28.png)

1. 向内 *收件人* 字段中，指定两个电子邮件地址。 最好使用与您的Acrobat Sign帐户无关联的电子邮件地址。
...
1. 在您的电子邮件中，会显示一条消息，其中包含用于查看和签名的链接。
   ...
   ![电子邮件屏幕截图](assets/automatelegal_31.png)

1. 选择 **[!UICONTROL 审阅并签名]**.
1. 选择 **[!UICONTROL 继续]** 接受使用条款。
1. 选择 **[!UICONTROL 开始]** 跳转到您需要签名的位置。

   ![开始标签的屏幕截图](assets/automatelegal_32.png)
...
1. 导航至 [Acrobat Sign REST文档](https://secure.na1.adobesign.com/public/docs/restapi/v6).
1. 展开 *transientDocuments* 以及 [POST/transientDocuments](https://benprojecteddemo.na1.adobesign.com/public/docs/restapi/v6#!/transientDocuments/createTransientDocument).
1. 选择 **[!UICONTROL OAUTH ACCESS-TOKEN]**.

   ![选择OAUTH ACCESS-TOKEN的位置的屏幕快照](assets/automatelegal_36.png)

1. 检查的OAUTH权限 *agreement_write*， *agreement_sign*， *widget_write*&#x200B;和 *library_write*.
1. 选择 **[!UICONTROL 授权]**.
...
要了解有关如何为Acrobat Sign创建授权令牌的更多信息，您可以按照概述的步骤操作 [此处](https://opensource.adobe.com/cn/acrobat-sign/developer_guide/helloworld.html).
...
1. 向内 *文件* 字段中，上传在之前步骤生成的PDF文档。
   ...
   ![上传PDF位置的屏幕截图](assets/automatelegal_37.png)

1. 选择 **[!UICONTROL 试用！]**.
1. 向内 **[!UICONTROL 响应正文]**，复制 *transientDocumentId* 值。
...
1. 由于可能需要额外的OAUTH范围，请选择 **[!UICONTROL OAUTH-ACCESS-TOKEN]** 再来一次。
...
1. 设置 **[!UICONTROL agreementId]** 到 *agreementId* 从上一个API调用提供。
1. 选择 **[!UICONTROL 立即试用！]**.
